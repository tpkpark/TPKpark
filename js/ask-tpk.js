...
(() => {
  ...
  function safeSource(source) {
    ...
    try {
      ...
      if (url.origin !== location.origin || !/^\/(?:ms\/|zh\/)?(?:about\/|home-living\/(?:(?:lavino|ga-hing|kuche-bath)\/)?|automotive\/|lifestyle\/(?:motd\/)?|leasing\/(?:shop-showroom\/|detached-building\/|semi-detached\/)?|contact\/|milestones\/|news\/|wong-shung-yen\/(?:public-record\/)?)?$/.test(url.pathname) || url.search || url.hash) return null;
