import { productionBudgetReady } from "./assistant-budget.mjs";
import { getVercelOidcToken } from "@vercel/oidc";
import { knowledgeByLocale, sources, sourceLinks } from "./assistant-knowledge.mjs";
import { propertyIds, propertyCatalog, pageContext, enquiryFields, cleanEnquiry, wantsEmailDraft } from "./assistant-rich.mjs";
...
export function replyLanguage(locale, messages, preference = "auto") {
  ...
  for (const message of [...messages].reverse().filter(item => item.role === "user")) {
    ...
    if (/\b(?:reply|answer) in (?:Japanese|日本語)\b|日本語で/iu.test(text)) return "ja";
...
Use only published characteristics when explaining a recommendation. Never invent popularity, returns, suitability for a licensed use, parking/loading capacity, fit-out approval, vacancy or viewing confirmation. A proposed business use is a visitor requirement, not a verified permitted use. Discuss strengths from published floor, area, rent, location or format. Do not invent translated proper names.
...
Only provide an enquiry object with requested=true when the latest user explicitly asks for an email/enquiry draft. Email draft requested this turn: ${Boolean(context.emailRequested)}. Otherwise requested=false and all fields empty. For businessType, budget, size, floor and timing, extract short EXACT QUOTATIONS from USER messages, not prior assistant suggestions. Use empty strings for missing facts. Do not include names, phone numbers, email addresses, passwords, personal health, payment details or URLs. A reviewable draft appears separately; the visitor edits it and sends from their email app. Do not claim to have sent anything, secured a unit or booked a viewing. Draft creation must not require providing personal details in chat.
...
export async function answerQuestion(input, { fetchImpl = fetch, token, env = process.env } = {}) {
  ...
  if (!productionBudgetReady(env)) throw new AssistantError("unavailable");
  // Production must use project-scoped OIDC, so an API key cannot bypass its budget.
  let auth = env.VERCEL_ENV === "production" ? undefined : token || env.AI_GATEWAY_API_KEY;
  if (!auth) {
  ...
  try {
    ...
    return { answer: parsed.answer.trim(), sources: sourceLinks(parsed.sourceIds, linkLocale), propertyIds: [...new Set(parsed.propertyIds)].filter(id => catalog[id]), enquiry: cleanEnquiry(parsed.enquiry, messages) };
  } catch { throw new AssistantError("unavailable"); }
}
