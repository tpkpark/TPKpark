...
- `lib/assistant-knowledge.mjs` derives the reference from every published website route and the leasing inventory, including Wong Shung Yen's profile, public-record chronology and the news archive. Answers use the published version in their own language, including names and role titles; shared media blocks contribute their published summaries and dates. No tenancy workbooks, inboxes or private documents are connected. New public content is picked up on the next deployment. Missing reference detail must not be described as proof that no information exists on the website. Inventory is not live; the team must confirm rents, suitability, availability and viewings. Unit 69 remains leased and its private agreed rent is excluded.
