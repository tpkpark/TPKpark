...
test("business guides provide the correct public branch contacts in all three languages", () => {
  for (const locale of ["en", "ms", "zh"]) {
    ...
    assert.doesNotMatch(sources.jaecooServiceCentre.texts[locale], /292.?6728|212.?2188|238.?4131|460.?9333|490.?3933|Residensi Park|Jalan Bandar Dua|Jalan Olivin/);
    ...
    assert.match(sources.techtricsAuto.texts[locale], /\+60 12 449 6696/);
    assert.match(sources.techtricsAuto.texts[locale], /info@mercedesworkshop\.com\.my/);
    assert.doesNotMatch(sources.techtricsAuto.texts[locale], /Techtunes|Techtra Academy|663 5454|TPK 1\/2/);
    assert.match(sources.kia4sService.texts[locale], /59, Jalan TPK 2\/8/);
    assert.match(sources.kia4sService.texts[locale], /\+60 3 8076 1005/);
    assert.doesNotMatch(sources.kia4sService.texts[locale], /KWM|47160|F-01-G/);
    assert.match(sources.mazdaKinrara.texts[locale], /8, Jalan TPK 2\/2/);
    assert.match(sources.mazdaKinrara.texts[locale], /\+60 3 8075 0812/);
    assert.match(sources.mazdaKinrara.texts[locale], /\+60 3 8075 0813/);
    assert.match(sources.mazdaKinrara.texts[locale], /4S/);
    assert.doesNotMatch(sources.mazdaKinrara.texts[locale], /Kota Damansara|6142 1662|Jalan TPK 2\/3/);
    ...
    assert.match(sources.nuarina.texts[locale], /41G, Jalan TPK 2\/8/);
    ...
    assert.match(sources.yummyNyonya.texts[locale], /43G, Jalan TPK 2\/8/);
    ...
    assert.match(sources.acesGymnasticAcademy.texts[locale], /11-1, Jalan TPK 2\/8/);
    ...
    assert.match(sources.forseeLens.texts[locale], /71, Jalan TPK 2\/8/);
    ...
    assert.deepEqual(sourceLinks(["totalTools"], locale).map(source => source.url), [routePath(locale, "totalTools")]);
    assert.match(sources.fagolli.texts[locale], /43-1, Jalan TPK 2\/8/);
    assert.match(sources.fagolli.texts[locale], /\+60 11 5407 8187/);
    assert.doesNotMatch(sources.fagolli.texts[locale], /1632 8187|1117 8187|E9-1/);
    assert.match(sources.fagolli.texts[locale], { en: /Saturday and Sunday visits are by appointment only/, ms: /Lawatan pada Sabtu dan Ahad adalah melalui janji temu sahaja/, zh: /星期六及星期日仅接受预约/ }[locale]);
    assert.deepEqual(sourceLinks(["fagolli"], locale).map(source => source.url), [routePath(locale, "fagolli")]);
