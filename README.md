...
- Upgrade details, validation and the required USD 5/month native Gateway project budget are documented in [Ask TPK Park upgrade](docs/ask-tpk-upgrade-2026-09-09.md). The `codex/ask-tpk-upgrade` preview enables the upgrade for review. Production requires the existing `TPK_AI_ENABLED=1` switch and a verified budget acknowledgement; the build blocks publication if the acknowledgement is missing. The acknowledgement is not itself a spending cap.
...
- The model remains `openai/gpt-4.1-mini` through Vercel AI Gateway. Production uses project OIDC. The local per-instance backstop is 100 requests per network per fixed 10-minute window, three concurrent calls, no automatic retries; the native project budget is the shared spend limit. No new service or paid subscription is introduced.
