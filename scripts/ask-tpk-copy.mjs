...
const upgradeCopy = {
  ...
  zh: {
    ...
    voiceStart: "语音输入问题", voiceStop: "停止录音", voiceListening: "正在聆听…", voiceReady: "已加入语音输入。请检查文字后再发送。", voiceDenied: "未获准使用麦克风。您仍可输入问题。", voiceNoSpeech: "未能识别语音。请点击麦克风重试。", voiceError: "目前无法使用语音输入。您仍可输入问题。", listen: "朗读", listenAnswer: "朗读此回答", stopAudio: "停止朗读",
    ...
    privacy: "发送问题会通过Vercel将本次对话分享给OpenAI以生成回答。请勿提供个人或机密资料。若您点击麦克风或“朗读”，语音识别及播放由您的浏览器或设备提供；可用性及处理方式取决于其设置和服务提供商。麦克风音频不会发送至TPK Park的分析工具。网站在本标签页的会话存储中保留最多三轮完整对话，跨页面有效，最长90分钟。点击“新对话”可清除；关闭标签页通常会清除会话存储。网站服务器不保留对话记录，也不会将对话或草稿内容发送至分析工具。使用保护仅保存临时计数和定期更换的网络哈希。服务提供商可能按其政策保留服务数据。AI回答可能不准确，请向团队确认当前供应及租赁详情。您须自行检查并发送电邮草稿；此对话不会发送咨询或确认看房预约。"
  }
};
