  ...
  return `<footer class="site-footer">
    <div class="shell">
      ...
      <div class="footer-bottom">
        <div class="footer-legal"><span>© 2026 ${escapeHtml(t.rights)}</span><button type="button" class="analytics-settings footer-privacy" data-analytics-settings aria-controls="analytics-panel" hidden>${escapeHtml(analyticsCopy[locale].settings)}</button></div>
        <span>Taman Perindustrian Kinrara · Puchong · Selangor</span>
      </div>
    </div>
  </footer>`;
}
...
function renderSplit(locale, block, index) {
  ...
  if (block.presentation === "renewal") {
    const credit = block.captionSource ? ` <a class="text-link" href="${escapeHtml(block.captionSource.url)}">${escapeHtml(block.captionSource.label)}</a>` : "";
    return `<section class="section section-sage renewal-section"><div class="shell renewal-grid">
      ...
      <figure class="renewal-figure"><img ${displayImage(block.image, "(max-width: 1020px) calc(100vw - 48px), (max-width: 1280px) 50vw, 580px")} alt="${escapeHtml(block.alt)}" loading="lazy" referrerpolicy="no-referrer">${block.caption ? `<figcaption>${escapeHtml(block.caption)}${credit}</figcaption>` : ""}</figure>
    </div></section>`;
  }
...
function askTpk(locale, routeId) {
  ...
  const config = { locales: Object.fromEntries(locales.map(key => [key, {
    copy: askTpkCopy[key],
    starters: starterQuestions(key, routeId),
    catalog: Object.fromEntries(Object.entries(propertyCatalog(key)).map(([id, card]) => [id, { ...card, image: imageAssets[card.image]?.src || card.image }])),
    sources: Object.fromEntries(routeIds.map(id => [id, { id, title: site[key].pages[id].eyebrow || site[key].pages[id].title, url: routePath(key, id) }]))
  }])) };
  const chat = `<div class="ask-tpk-language"><label for="ask-tpk-language" data-ask-copy="language">${escapeHtml(copy.language)}</label><select id="ask-tpk-language" data-ask-language><option value="auto" data-ask-copy="auto">${escapeHtml(copy.auto)}</option><option value="en">EN</option><option value="zh">中文</option><option value="ms">BM</option></select></div>
      <p class="ask-tpk-intro" id="ask-tpk-intro" data-ask-copy="aiIntro">${escapeHtml(copy.aiIntro)}</p>
      <div class="ask-tpk-messages" data-ask-messages role="log" aria-live="polite" aria-relevant="additions" data-ask-copy-label="label" aria-label="${escapeHtml(copy.label)}"></div>
      ...
      <form class="ask-tpk-form" data-ask-form>
        <label class="sr-only" for="ask-tpk-question" data-ask-copy="input">${escapeHtml(copy.input)}</label>
        <textarea id="ask-tpk-question" data-ask-input name="question" rows="2" maxlength="2000" required autocomplete="off" data-ask-copy-placeholder="placeholder" placeholder="${escapeHtml(copy.placeholder)}"></textarea>
        <p class="ask-tpk-voice-status" id="ask-tpk-voice-status" data-ask-voice-status role="status" hidden></p>
        <div class="ask-tpk-actions"><button type="button" data-ask-clear data-ask-copy="clear">${escapeHtml(copy.clear)}</button><div class="ask-tpk-compose-actions"><button class="ask-tpk-voice" type="button" data-ask-voice aria-label="${escapeHtml(copy.voiceStart)}" aria-describedby="ask-tpk-voice-status" aria-pressed="false" title="${escapeHtml(copy.voiceStart)}" hidden><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="2" width="6" height="12" rx="3"/><path d="M5 10a7 7 0 0 0 14 0M12 17v5M8 22h8"/></svg></button><button class="ask-tpk-send" type="submit" data-ask-send><span data-ask-copy="send">${escapeHtml(copy.send)}</span> <span aria-hidden="true">↑</span></button></div></div>
      </form>
      <div class="ask-tpk-handoff"><a href="tel:+60380765200" data-ask-copy="call">${escapeHtml(copy.call)}</a><a href="mailto:info@tpkpark.com" data-ask-copy="email">${escapeHtml(copy.email)}</a><a href="${routePath(locale, "leasing")}" data-ask-copy="browse" data-ask-route="leasing">${escapeHtml(copy.browse)}</a><button type="button" data-ask-draft data-ask-copy="draftButton">${escapeHtml(copy.draftButton)}</button></div>`;
  return `<aside class="ask-tpk" data-ask-tpk data-locale="${locale}" data-ai-enabled="${ai}" data-pathname="${routePath(locale, routeId)}" data-you="${escapeHtml(copy.you)}" data-thinking="${escapeHtml(copy.thinking)}" data-error="${escapeHtml(copy.error)}" data-busy="${escapeHtml(copy.busy)}" data-sources="${escapeHtml(copy.sources)}" data-assistant="${escapeHtml(copy.aiLabel)}">
    ...
    <section class="ask-tpk-panel${ai ? " ask-tpk-ai" : ""}" id="ask-tpk-panel" data-ask-panel lang="${locale}" role="dialog" aria-labelledby="ask-tpk-title" aria-describedby="ask-tpk-intro" tabindex="-1" hidden>
      <div class="ask-tpk-heading"><div class="ask-tpk-identity"><h2 id="ask-tpk-title" data-ask-copy="label">${escapeHtml(copy.label)}</h2>${ai ? `<details class="ask-tpk-guide"><summary><span data-ask-copy="aiLabel">${escapeHtml(copy.aiLabel)}</span><span class="ask-tpk-guide-icon" aria-hidden="true">i</span></summary><p class="ask-tpk-guide-note" data-ask-copy="privacy">${escapeHtml(copy.privacy)}</p></details>` : ""}</div><button type="button" class="ask-tpk-close" data-ask-close data-ask-copy-label="close" aria-label="${escapeHtml(copy.close)}"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button></div>
