...
function askTpk(locale, routeId) {
  ...
  const chat = `<div class="ask-tpk-language"><label for="ask-tpk-language">${escapeHtml(copy.language)}</label><select id="ask-tpk-language" data-ask-language><option value="auto">${escapeHtml(copy.auto)}</option><option value="en">EN</option><option value="zh">中文</option><option value="ms">BM</option></select></div>
  ...
  return `<aside class="ask-tpk" data-ask-tpk data-locale="${locale}" data-ai-enabled="${ai}" data-pathname="${routePath(locale, routeId)}" data-you="${escapeHtml(copy.you)}" data-thinking="${escapeHtml(copy.thinking)}" data-error="${escapeHtml(copy.error)}" data-busy="${escapeHtml(copy.busy)}" data-sources="${escapeHtml(copy.sources)}" data-assistant="${escapeHtml(copy.aiLabel)}">
    ...
    <section class="ask-tpk-panel${ai ? " ask-tpk-ai" : ""}" id="ask-tpk-panel" data-ask-panel role="dialog" aria-labelledby="ask-tpk-title" aria-describedby="ask-tpk-intro" tabindex="-1" hidden>
      <div class="ask-tpk-heading"><div class="ask-tpk-identity"><h2 id="ask-tpk-title">${escapeHtml(copy.label)}</h2>${ai ? `<details class="ask-tpk-guide"><summary><span>${escapeHtml(copy.aiLabel)}</span><span class="ask-tpk-guide-icon" aria-hidden="true">i</span></summary><p class="ask-tpk-guide-note">${escapeHtml(copy.privacy)}</p></details>` : ""}</div><button type="button" class="ask-tpk-close" data-ask-close aria-label="${escapeHtml(copy.close)}"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18"/></svg></button></div>
