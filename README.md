...
- `api/ask.js` calls OpenAI GPT-4.1 mini through Vercel AI Gateway. The existing server-side `AI_GATEWAY_API_KEY`, when configured, takes precedence; otherwise `@vercel/oidc` obtains the current deployment identity inside each request. No provider key is shipped to the browser. Do not put secrets in chat, source code or GitHub.
- `lib/assistant-knowledge.mjs` derives the reference from every published website route and the leasing inventory, including Wong Shung Yen's profile, public-record chronology and the news archive. Answers use the published version in their own language, including names and role titles; shared media blocks contribute their published summaries and dates. Shared rental guides, property cautions, floor-plan notes and enquiry-form instructions are included from the same public content used to render each page. Analytics/privacy explanations are excluded from the reference as requested; existing analytics privacy controls are unchanged; the assistant disclosure now describes its limited tab session memory. Floor-plan images, full PDFs and full external articles are not read. No tenancy workbooks, inboxes or private documents are connected. New public content is picked up on the next deployment. Missing reference detail must not be described as proof that no information exists on the website. Inventory is not live; the team must confirm rents, suitability, availability and viewings. Unit 69 remains leased and its private agreed rent is excluded.
- The endpoint does not save or log transcripts; `store: false` is sent to the model. Messages still pass through Vercel and OpenAI, whose service retention policies apply. Chat content never enters site analytics. This is not a zero-retention guarantee.
- The assistant cannot send emails, collect contact details, book viewings or agree tenancy terms. On explicit request it prepares an editable email draft from visitor-supplied requirements; the visitor reviews and sends it.
...
- Upgrade details and validation are documented in [Ask TPK Park upgrade](docs/ask-tpk-upgrade-2026-09-09.md). The `codex/ask-tpk-upgrade` preview enables the upgrade for review. Production retains the existing `TPK_AI_ENABLED=1` switch. At the owner's request, hosting and AI usage settings remain with Lawrence; no US$5 monthly budget or new release acknowledgement is required by this upgrade.
- Conversations retain at most three completed exchanges in same-tab session storage for up to 90 minutes of inactivity. New chat clears them. No server transcripts, analytics text, private records or visitor-contact collection. Storage-blocked browsers keep page memory only.
- Auto / EN / 中文 / BM reply preferences, page-aware starter questions, approved property cards and current brochure links accompany the public-reference answers. Office telephone and email remain the human handoff channels.
- The model remains `openai/gpt-4.1-mini` through Vercel AI Gateway with the existing authentication setup. The local per-instance backstop is 100 requests per network per fixed 10-minute window, three concurrent calls and no automatic retries. These are abuse controls, not a monthly spend cap. Existing Gateway quota refusals are handled safely. No new service or paid subscription is introduced.
- `TPK_AI_ENABLED=1 npm test` runs deterministic validation without model calls. The named preview build runs seven fixed public questions and writes `assets/assistant-preview-check.json`. Inspect that report before release; these preview calls consume Gateway credit.
...
- **Detailed analytics:** Following the 9 September search/enquiry recommendations, new visitors receive basic Vercel statistics without a first-visit dialog. GA4 is optional from Privacy & analytics. This restores visitor choices removed by PR #28. Existing saved choices and legacy refusals remain respected. Enhanced measurement remains disabled in the stream to avoid duplicate or unintended form/link collection. GA4 adds cookie-based visit analysis, form-start counts and scroll milestones (50% and 90%, once each per page). Advertising features remain disabled. Events are not replayed when a visitor changes their setting.
...
- Ask TPK Park also emits `assistant_open`, `assistant_question`, `assistant_answer`, `assistant_error` and `assistant_draft_ready`. A question is counted only when a request starts; answers/drafts are counted only on a new successful response, never on session restoration. Error reasons are restricted to five fixed categories. No message text, arbitrary diagnostic, model output or draft contents are included.
- Link clicks within Ask TPK Park retain their normal event name and use an `assistant:` prefix in the existing Interaction target report field, for example `assistant:phone`, `assistant:email` and `assistant:email_draft`. They are counted once, not duplicated as a second conversion. Property and PDF targets follow the same convention. See `docs/search-and-enquiry-measurement.md` for the baseline and review method.
- The homepage uses real frontage photographs from `assets/images/`, with text alongside the images and full photographic frames preserved. The three cluster cards use text to avoid repeating these photographs. The billboard mock-up is artwork and must not be presented as a real park photograph. `scripts/image-assets.mjs` maps existing assets; social preview image URLs are maintained separately from page imagery.
- The lifestyle frontage display photograph uses the owner-approved asphalt retouch of 11 September 2026, encoded at 480 and 1600 pixels in `lifestyle-frontage-20260911-*.webp`. The original photographs remain available; this is a photographic retouch, not a record of completed resurfacing work.
