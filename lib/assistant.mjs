...
export function assistantEnabled(env = process.env) {
  ...
  return env.TPK_AI_ENABLED === "1" || (env.VERCEL_ENV === "preview" && ["codex/ai-assistant", "codex/ask-tpk-upgrade", "codex/ask-tpk-voice", "codex/ask-tpk-disclosure-cleanup", "codex/ask-tpk-ai-disclosure", "codex/ask-tpk-chat-language", "codex/ask-tpk-mandarin-audio"].includes(env.VERCEL_GIT_COMMIT_REF));
}
...
export async function answerQuestion(input, { fetchImpl = fetch, token, env = process.env } = {}) {
  ...
  try {
    ...
    return { answer, sources: sourceLinks(ids, linkLocale), propertyIds: selectedIds, enquiry, language };
